...
def test_make_tweet_text(twitter):
    ...
    assert twitter.make_tweet_text(
        [
            {"name": "Lockheed Martin", "sentiment": -0.1, "ticker": "LMT"},
            {"name": "Boeing", "sentiment": 0.1, "ticker": "BA"},
        ],
    ...
    assert twitter.make_tweet_text(
        [
            {"ticker": chr(i - 32), "name": chr(i), "sentiment": 0}
...
def test_gnlp_sentiment_emoji(twitter):
    assert twitter.gnlp_sentiment_emoji(0.5) == "\U0001f44d"
    assert twitter.gnlp_sentiment_emoji(-0.5) == "\U0001f44e"
    assert twitter.gnlp_sentiment_emoji(0) == "¯\\_(\u30c4)_/¯"
    assert twitter.gnlp_sentiment_emoji(None) == "¯\\_(\u30c4)_/¯"
